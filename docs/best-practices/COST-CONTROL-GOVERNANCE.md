...
CostGovernanceRoles:
  ...
  FinanceAdmin:
    ...
    responsibilities: ['Monthly budget reviews', 'Cost optimization recommendations']
    
  ServiceOwner:
    permissions: ['service-budget-monitoring', 'optimization-implementation']
    responsibilities: ['Service-level cost optimization', 'Usage pattern analysis']
...
export class CostAnalytics {
  ...
  async trackServiceCosts(): Promise<ServiceCostReport> {
    const services = [
      'ai-generation', 'infrastructure', 'data-storage', 
    ...
    return {
      ...
      recommendations: await this.generateOptimizationRecommendations(serviceCosts)
    };
  }
  ...
  async trackAICosts(): Promise<AICostReport> {
    ...
    const totalTokens = aiMetrics.reduce((sum, metric) => sum + metric.tokensUsed, 0);
  ...
  private async getAIProviderCosts(provider: string): Promise<AIProviderCost> {
    ...
    const usage = await this.dynamodb.query({
      TableName: process.env.AI_USAGE_TABLE!,
      IndexName: 'provider-timestamp-index',
      KeyConditionExpression: 'provider = :provider AND #timestamp >= :startTime',
    ...
    const tokensUsed = usage.Items?.reduce((sum, item) => sum + (item.tokens_used || 0), 0) || 0;
    const requests = usage.Items?.length || 0;
    const cost = usage.Items?.reduce((sum, item) => sum + (item.cost || 0), 0) || 0;
  ...
  async trackUserCosts(userId: string): Promise<UserCostReport> {
    ...
    const totalCost = aiUsage.cost + storageUsage.cost + userActivity.infrastructureCost;
    ...
    return {
      ...
      recommendations: await this.getUserOptimizationRecommendations(userId, totalCost)
    };
  }
  ...
  async enforceBudgetLimits(): Promise<BudgetEnforcementResult> {
    ...
    for (const budget of budgets) {
      const utilizationPercent = (currentSpending[budget.category] / budget.limit) * 100;
      ...
      } else if (utilizationPercent >= budget.alertThresholds[budget.alertThresholds.length - 1]) {
    ...
    return {
      timestamp: new Date(),
      enforcements,
      totalBudgetUtilization: this.calculateOverallUtilization(budgets, currentSpending),
      recommendations: await this.getBudgetRecommendations(budgets, currentSpending)
    };
  }

  // Cost optimization recommendations
  async generateOptimizationRecommendations(serviceCosts: ServiceCost[]): Promise<OptimizationRecommendation[]> {
    ...
    if (aiCosts && aiCosts.efficiency < 0.7) {
      recommendations.push({
        ...
        description: 'Switch to more cost-effective models for simple story generation',
    ...
    const infraCosts = serviceCosts.find(s => s.serviceName === 'infrastructure');
    ...
    const storageCosts = serviceCosts.find(s => s.serviceName === 'data-storage');
    if (storageCosts && storageCosts.monthlyProjection > 5000) {
      recommendations.push({
        category: 'storage-optimization',
        title: 'Implement Data Lifecycle Management',
        description: 'Automatically archive old story data to reduce storage costs',
  ...
  private async getServiceUsageMetrics(serviceName: string): Promise<Record<string, number>> {
    // Get service-specific usage metrics
    return {};
  }

  private async calculateServiceEfficiency(serviceName: string): Promise<number> {
    // Calculate efficiency score (0-1) based on cost vs. performance
    return 0.8;
  }

  private async getServiceTrends(serviceName: string): Promise<{ direction: string; changePercent: number }> {
  ...
  private calculateUserEfficiency(totalCost: number, userActivity: any): number {
  ...
  private async getUserOptimizationRecommendations(userId: string, totalCost: number): Promise<string[]> {
  ...
  private async sendBudgetAlert(budget: any, utilizationPercent: number): Promise<void> {
    // Send budget alert notifications
  }

  private calculateOverallUtilization(budgets: any[], currentSpending: any): number {
    // Calculate overall budget utilization
    return 0.75;
  }

  private async getBudgetRecommendations(budgets: any[], currentSpending: any): Promise<string[]> {
