  ...
  testPrompts.forEach(seed => {
    describe(`Seed: ${seed}`, () => {
      it('should generate consistent opening for same seed', async () => {
        ...
        expect(calculateSimilarity(narrative1, narrative2)).toBeGreaterThan(0.7);
      });
...
test.describe('Story Progression', () => {
  test('should complete a full story journey', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="narrative"]')).toContainText('(Y/N)');
    ...
    await expect(page.locator('[data-testid="narrative"]')).toContainText('(Y/N)', { timeout: 10000 });
    
    // Continue story for several beats
    for (let i = 0; i < 5; i++) {
      const choice = Math.random() > 0.5 ? 'choice-yes' : 'choice-no';
      await page.click(`[data-testid="${choice}"]`);
      await page.waitForSelector('[data-testid="thinking"]', { state: 'visible' });
      await page.waitForSelector('[data-testid="thinking"]', { state: 'hidden' });
    }
    
    // Story should maintain coherence
    const narrativeElements = await page.locator('[data-testid="log-entry"]').count();
  ...
  test('should handle story restart', async ({ page }) => {
    ...
    while (!(await page.locator('text=(Restart?)').isVisible())) {
      if (await page.locator('[data-testid="choice-yes"]').isVisible()) {
        await page.click('[data-testid="choice-yes"]');
        await page.waitForSelector('[data-testid="thinking"]', { state: 'hidden', timeout: 10000 });
      } else {
  ...
  test('should maintain seed consistency @smoke', async ({ page }) => {
    ...
    const firstNarrative = await page.locator('[data-testid="narrative"]').textContent();
    
    // Refresh page with same seed
    await page.reload();
    
    const secondNarrative = await page.locator('[data-testid="narrative"]').textContent();
...
test.describe('Accessibility', () => {
  ...
  test('should have proper ARIA labels', async ({ page }) => {
    ...
    // Check button labels
    await expect(page.locator('[data-testid="choice-yes"]')).toHaveAttribute('aria-label', 'Choose Yes');
    await expect(page.locator('[data-testid="choice-no"]')).toHaveAttribute('aria-label', 'Choose No');
  });
});
```
